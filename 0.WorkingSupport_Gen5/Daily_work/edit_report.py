...
class ToolTip:
    ...
    def show_tooltip(self, event=None):
        try:
            ...
            elif isinstance(self.widget, tk.Listbox):
                # Lấy vị trí item đang active, nếu không thì đặt mặc định
                active = self.widget.curselection()
                if active:
                    x, y, _, _ = self.widget.bbox(active[0])
                else:
                    x, y = 0, 0
            else:
                x, y = 0, 0
...
def browse_directory(app):
    directory = filedialog.askdirectory()
    if directory:
        app.directory_entry.delete(0, tk.END)
        app.directory_entry.insert(0, directory)
        list_files()

def browse_excel_file(app):
    filepath = filedialog.askopenfilename(filetypes=[("Excel files", "*.xlsx *.xls")])
    if filepath:
        app.excel_entry.delete(0, tk.END)
        app.excel_entry.insert(0, filepath)

def list_files(app):
    folder_path = app.directory_entry.get()
    extension = app.ext_entry.get()
    if not folder_path or not extension:
        return
    files = [f for f in os.listdir(folder_path) if f.endswith(extension)]
    app.current_files_listbox.delete(0, tk.END)
    for file in files:
        file_name = os.path.splitext(file)[0]
        app.current_files_listbox.insert(tk.END, file_name)
    update_preview_list()

def update_preview_list(app):
    app.preview_files_listbox.delete(0, tk.END)
    folder_path = app.directory_entry.get()
    extension = app.ext_entry.get()
    rename_type = app.rename_var.get()
    
    if app.excel_var.get():
        excel_path = app.excel_entry.get()
        if not excel_path:
            return
        try:
        ...
        for file in os.listdir(folder_path):
            if file.endswith(extension):
                ...
                new_name += file_ext
                app.preview_files_listbox.insert(tk.END, new_name)
    else:
        add_remove_text = app.add_remove_entry.get()
        replace_from_text = app.replace_from_entry.get()
        replace_to_text = app.replace_to_entry.get()
